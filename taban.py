...
def takip_kontrol():
    ...
    for k in liste:
        ...
        telegram_gonder(
            f"📊 TABAN STRATEJİSİ 48-SAAT SONUCU\n"
            f"🏢 {k.get('desc', '')} ({k['kod']})\n"
            f"📉 Adayken: {k['degisim']:+.1f}% | baz {baz:.2f} TL\n"
            f"💰 48s kapanış: {son:.2f} TL\n"
            f"{emoji} Sonuç: {fark:+.2f}%\n"
            f"📒 Deftere işlendi: haftalık karnede 'Taban dönüş' hanesine yazılacak.")
        k["son"] = son
        k["fark"] = round(fark, 2)
        k["done"] = True
        degisti = True
...
def tur(manuel=False):
    ...
    for item in adaylar:
        ...
        haber_txt = "VAR → düşüşün bir haber nedeni olabilir" if haber_var else "YOK → düşüş habersiz (panik/sektör); dönüş ihtimali daha anlamlı"
        telegram_gonder(
            f"🔄 TABAN DÖNÜŞ ADAYI\n"
            f"🏢 {desc} ({kod})\n"
            f"📉 {change:+.1f}% | Fiyat: {close} TL | RSI: {rsi:.0f} | Göreceli hacim: {rvol:.1f}x\n"
            f"📰 KAP (24s): {haber_txt}\n"
            f"🎯 Strateji: RSI 25-50 + hacim ilgisi = erken dönüş adayı; stop disiplini şart.\n"
